document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalTitle = termsModal.querySelector('h2'); // 모달 타이틀 요소 추가
    ...
    const termsData = {
        ...
        terms2: {
            ...
            content: `
                ...
                <p>동의하신 후에도 무료거부전화(080-870-1940) 또는 마이페이지를 통해 언제든지 철회하실 수 있습니다.</p>
            `
        }
    ...
    viewTermsLinks.forEach(link => {
        link.addEventListener('click', function(event) {
            ...
            const termsInfo = termsData[termsId];

            if (termsInfo) {
                modalTitle.textContent = termsInfo.title; // 팝업 타이틀 설정
                termsContentDiv.innerHTML = termsInfo.content; // 약관 내용 삽입
            } else {
                modalTitle.textContent = '정보 없음';
                termsContentDiv.innerHTML = '<p>약관 내용을 불러올 수 없습니다.</p>';
            }
